...
import { parseMzXML } from '../parseMzXML.js';

const pathFiles = join(import.meta.dirname, 'data');

test('read 32 bits mzXML test file', async () => {
...
test.todo('read 32 bits mzXML compressed test file', async () => {
  ...
  expect(response.series.ms.data[0][0][0]).toBe(-2);
  expect(response.series.ms.data[0][1][0]).toBe(0);
});

test('read 64 bits mzXML test file', async () => {
