...
test('read tiny.mzML as node blob', async () => {
...
test('read test.mzML', async () => {
  ...
  expect(response.times.slice(0, 6)).toStrictEqual([0, 0.2, 0.4, 0.6, 0.8, 1]);
...
test('read compressed 32bits', async () => {
  ...
  expect(response.times.slice(0, 6)).toStrictEqual([
    0.004935, 0.007896666666666666, 0.011218333333333334, 0.022838333333333332,
    0.034925, 0.04862,
  ]);
